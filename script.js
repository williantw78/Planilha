// ESTRUTURA CENTRAL DE DADOS DO SISTEMA (Inicia completamente vazia)
let appData = {
    users: [],
    months: [], 
    expenses: [],
...
function renderAll() {
    ...
    // O Dashboard e as Tabelas dependem diretamente do mês selecionado
    updateDashboard();
...
// Atualiza todas as caixas de seleção (dropdowns) de responsáveis na página de configuração
...
function addMonth() {
    ...
    // Se já existia um mês anterior, copia os valores dos serviços "recorrentes" para o novo mês
    if (lastActiveMonth) {
...
function renderMonthsSelector() {
    ...
    // Mantém o mês selecionado ou escolhe o mais recente criado por padrão
...
function renderExpensesTable(activeMonth) {
    ...
    if (!activeMonth || activeMonth === "Nenhum mês criado" || activeMonth === "") {
        thead.innerHTML = "<tr><th>Serviços</th><th>Nenhum mês selecionado</th></tr>";
        tbody.innerHTML = "<tr><td colspan='2' style='text-align:center; padding: 20px; color: #64748b;'>Crie e selecione um mês na página de configurações para começar.</td></tr>";
        tfoot.innerHTML = "";
        return;
    }
    ...
    if (appData.expenses.length === 0) {
        tbody.innerHTML = `<tr><td colspan="2" style="text-align:center; padding: 20px; color: #64748b;">Nenhum serviço cadastrado. Vai a "Gerir Dados" para adicionar.</td></tr>`;
        tfoot.innerHTML = "";
        return;
    }
...
function renderIncomesTable(activeMonth) {
    ...
    if (appData.incomes.length === 0) {
        tbody.innerHTML = `<tr><td colspan="2" style="text-align:center; padding: 20px; color: #64748b;">Nenhuma receita cadastrada. Vai a "Gerir Dados" para adicionar.</td></tr>`;
        tfoot.innerHTML = "";
        return;
    }
...
// DASHBOARD, RESUMO MENSAL E ATUALIZADOR DE FILTROS
function updateDashboard() {
    ...
    if (!m || m === "Nenhum mês criado" || m === "") {
        ...
        if (tableBreakdown) tableBreakdown.querySelector("tbody").innerHTML = "<tr><td colspan='4' style='text-align:center; color:#64748b;'>Sem dados.</td></tr>";
        return;
    }
    ...
    if (tableBreakdown) {
        ...
        if (appData.users.length === 0) {
            tbody.innerHTML = "<tr><td colspan='4' style='text-align:center; color:#64748b;'>Nenhum responsável criado.</td></tr>";
            return;
        }
...
// --- INTEGRAÇÃO COM GITHUB E BACKUPS ---
